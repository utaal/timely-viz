...
use timely::logging::{WorkerIdentifier, TimelyEvent};
...
/// timely dataflow.
pub fn make_replayers(sockets: Arc<Mutex<Vec<Option<TcpStream>>>>, index: usize, peers: usize) -> Vec<EventReader<Product<RootTimestamp, Duration>, (Duration, WorkerIdentifier, TimelyEvent), TcpStream>> {

    sockets.lock().unwrap()
        .iter_mut().enumerate()
        .filter(|(i, _)| *i % peers == index)
        .map(move |(_, s)| s.take().unwrap())
        .map(|r| EventReader::<Product<RootTimestamp, Duration>, (Duration, WorkerIdentifier, TimelyEvent),_>::new(r))
        .collect::<Vec<_>>()
}
